...
```
注意，第一个状态，<b>必须指定controller</b>，controller为顶层VM的`$id`。<br>
注意，添加状态的顺序，必须先添加aaa, 再添加aaa.bbb，再添加aaa.bbb.ccc，不能先添加aaa.bbb，再添加aaa<br>
4、启动历史管理器
```javascript
    avalon.history.start({
        basepath: "/mmRouter"
...
```javascript
   avalon.scan()
```
avalon.state的参数与配置项与内部生成属性<br>
```javascript
avalon.state(stateName: opts)
```

* stateName： 指定当前状态名<br>
* url:  当前状态对应的路径规则，与祖先状态们组成一个完整的匹配规则<br>
* controller： 指定当前所在的VM的名字（如果是顶级状态对象，必须指定）<br>
...
```
views的每个键名(keyname)的结构为viewname@statename，<br>
如果名字不存在@，则viewname直接为keyname，statename为opts.stateName<br>
如果名字存在@, viewname为match[0], statename为match[1]<br>
* template: 指定当前模板，也可以为一个函数，传入opts.params作参数<br>
* templateUrl: 指定当前模板的路径，也可以为一个函数，传入opts.params作参数<br>
* templateProvider: 指定当前模板的提供者，它可以是一个Promise，也可以为一个函数，传入opts.params作参数<br>
* onChange: 当切换为当前状态时调用的回调，this指向状态对象，参数为匹配的参数，<br>
          我们可以在此方法 定义此模板用到的VM， 或修改VM的属性<br>
* onBeforeLoad: 模板还没有插入DOM树执行的回调，this指向[ms-view]元素节点，参数为状态对象<br>
* onAfterLoad: 模板插入DOM树执行的回调，this指向[ms-view]元素节点，参数为状态对象<br>
* abstract:  表示它不参与匹配<br>
* parentState: 父状态对象（框架内部生成）<br>
 */